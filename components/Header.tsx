    ...
    <header className="sticky top-0 z-30 glass-panel border-b border-slate-200 dark:border-slate-800/80 backdrop-blur-xl transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-center md:justify-end gap-2 sm:gap-2.5">
          ...
          {setSelectedCountry && (
            <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-full px-2.5 py-1 text-xs shadow-sm shrink-0">
          ...
          {onOpenMacroCalculator && (
            <button
              ...
              type="button"
              className="flex items-center gap-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-3 py-1.5 rounded-full text-xs font-semibold shadow-sm transition-all hover:scale-[1.02] active:scale-[0.98] shrink-0"
              aria-label="Open Daily Calorie & Macro Calculator"
            >
              <Flame className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
              <span className="hidden sm:inline">Daily Calorie & Macro Target</span>
              <span className="inline sm:hidden">Macro Targets</span>
            </button>
          )}

          {/* Active Profile Pill */}
          <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-full px-3 py-1.5 text-xs text-slate-700 dark:text-slate-300 shrink-0">
          ...
          <div className="flex items-center bg-slate-200/90 dark:bg-slate-900 p-1 rounded-xl border border-slate-300 dark:border-slate-800 text-xs font-semibold shadow-inner shrink-0">
            <button
            ...
            >
              <Sun className={`w-3.5 h-3.5 ${theme === 'light' ? 'text-slate-950 stroke-[2.5]' : 'text-amber-500'}`} />
              <span>Light</span>
            </button>
            <button
            ...
            >
              <Moon className={`w-3.5 h-3.5 ${theme === 'dark' ? 'text-slate-950 stroke-[2.5]' : 'text-indigo-400'}`} />
              <span>Dark</span>
            </button>
          </div>
          ...
          <button
            ...
            type="button"
            className="flex items-center gap-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white px-3.5 py-1.5 rounded-xl text-xs font-semibold shadow-md shadow-emerald-600/20 transition-all hover:scale-[1.02] active:scale-[0.98] shrink-0"
          >
