...
import { ShieldCheck, SlidersHorizontal, Sparkles, ChevronDown, Globe, Loader2, Flame, Dumbbell, Droplets } from 'lucide-react';
        ...
        <section className="glass-panel p-5 rounded-3xl border border-slate-200 dark:border-slate-800/80 bg-gradient-to-r from-emerald-500/10 via-slate-100 to-teal-500/10 dark:from-slate-900/90 dark:via-slate-900/50 dark:to-emerald-950/20 relative overflow-hidden shadow-sm">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto">
              <div className="flex flex-wrap items-center gap-2 bg-white/80 dark:bg-slate-950/60 p-2.5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              ...
              {savedMacroTargets && (
                <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/90 dark:bg-slate-950 text-white p-2.5 rounded-2xl border border-emerald-500/30 shadow-md text-xs">
                  <div className="flex items-center gap-1.5 text-emerald-400 font-bold">
                    <Flame className="w-3.5 h-3.5" />
                    <span>Target: {savedMacroTargets.targetCalories} kcal</span>
                  </div>
                  <div className="flex items-center gap-3 text-[11px] text-slate-300">
                    <span className="flex items-center gap-1">
                      <Dumbbell className="w-3 h-3 text-teal-400" />
                      Protein: {savedMacroTargets.proteinGrams}g
                    </span>
                    <span className="flex items-center gap-1">
                      <Droplets className="w-3 h-3 text-blue-400" />
                      Water: {savedMacroTargets.waterLiters}L
                    </span>
                  </div>
                  <button
        ...
        <section className="space-y-2">
          ...
          {isSearchingAPI && (
            <div className="flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400 py-1">
              ...
              <span>
                Searching global dish & food databases for {selectedCountry.flag} {selectedCountry.name}...
              </span>
            </div>
          )}
          {apiProducts.length > 0 && searchQuery && (
            <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 py-0.5">
              ...
              <span>
                Found <strong className="text-emerald-600 dark:text-emerald-400">{apiProducts.length}</strong> items & dishes for "{searchQuery}"
              </span>
            </div>
          )}
        </section>
          ...
          {filteredProducts.length === 0 ? (
            <div className="glass-panel text-center py-16 px-4 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-3">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                Try searching for standard commercial brands or cooked dishes, select a different country, or reset your safety status filters.
              </p>
              <button
                onClick={() => {
                  setSearchQuery('');
